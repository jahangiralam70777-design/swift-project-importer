...
function tryLabeledParse(text: string): { mcq: ParsedMcq | null; reason?: string } | null {
  const hasQuestionLabel = /^\s*(?:q|question)\s*[:.\-)]\s*$/im.test(text)
    || /^\s*(?:q|question)\s*[:.\-)]\s+\S/im.test(text);
  const hasTrueLine = /^\s*true\s*$/im.test(text);
  const hasFalseLine = /^\s*false\s*$/im.test(text);
  const hasAnswerLabel = /^\s*(?:correct\s+answer|correct\s+option|answer|ans)\s*[:.\-)]/im.test(text);
  ...
  let current: { kind: "bucket" | "option"; ref: string[] } = { kind: "bucket", ref: buckets.prelude };
  ...
  const explanationText = buckets.explanation.join("\n").replace(/^\n+|\n+$/g, "").trimEnd();
  ...
  const tfOptions =
    options.length === 2 &&
    options.every((o) =>
      /^(true|false)$/i.test(o.lines.join(" ").trim()),
    );
  if (tfOptions) {
    ...
    if (!correct) return { mcq: null, reason: `Could not resolve True/False answer "${answerRaw}"` };
    return {
      mcq: {
